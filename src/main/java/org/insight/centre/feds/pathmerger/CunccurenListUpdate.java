...
public class CunccurenListUpdate {
	...
	public Collection<List<Edge<Node, Node>>> getCompletePath(CopyOnWriteArraySet<String> d1,
			org.apache.jena.sparql.path.Path pathExp) throws Exception {
		...
		if (pathExp != null) {
			PathArbiter<Node, Node> parb = new RegExpPathArbiter<>(RDFPathExprFactory.createPathExpr(pathExp));
			bidir.setPathArbiter(parb);
		}

		Collection<List<Edge<Node, Node>>> results = bidir.run(ResourceFactory.createResource(this.startNode).asNode(),
				ResourceFactory.createResource(this.targetNode).asNode(), 32);
		return results;
	}
		...
		for (String comList : d1) {
			...
			for (String rpeat : d1) {
				if (!rpeat.equals(comList)) {
					...
					// if (d1.indexOf(comList) != (d1.lastIndexOf(d1.size() -
					// 1))){
