...
/*String LOCAL_ENDPOINT="http://localhost:3040/d1/query";
	String ENDPOINT1="http://localhost:3041/d2/query";
	String ENDPOINT2= "http://localhost:3042/d3/query";*/
...
/*	String LOCAL_ENDPOINT="http://10.196.2.224:3001/ev1/query";
    String ENDPOINT1="http://10.196.2.224:3002/ev2/query";
    String ENDPOINT2="http://10.196.2.224:3003/ev3/query";
    String ENDPOINT3="http://10.196.2.224:3004/ev4/query";
    String ENDPOINT4="http://10.196.2.224:3005/ev5/query";
    String ENDPOINT5="http://10.196.2.224:3006/ev6/query";
    String ENDPOINT6="http://10.196.2.224:3007/ev7/query";
    String ENDPOINT7="http://10.196.2.224:3008/ev8/query";
    String ENDPOINT8="http://10.196.2.224:3009/ev9/query";
    String ENDPOINT9="http://10.196.2.224:3010/ev10/query";*/
...
/*	String LOCAL_ENDPOINT="http://localhost:3055/disease/query";
	...
	public Collection<List<Edge<V,E>>> run(Node start, Node end, Node filter,  int k,org.apache.jena.sparql.path.Path path) throws Exception {
	...
	/*	beforeFilterEndp.add(ENDPOINT4);
		beforeFilterEndp.add(ENDPOINT5);
		beforeFilterEndp.add(ENDPOINT6);
		beforeFilterEndp.add(ENDPOINT7);
		beforeFilterEndp.add(ENDPOINT8);
		beforeFilterEndp.add(ENDPOINT9);*/
	...
	private Collection<List<Edge<V,E>>> queueItr(int k) throws Exception {
		...
		while (!q.isEmpty()) {
			...
			visit(current);
			if (solutions.size() >= k || q.size()>10000) {
		...
		// write federated paths to file 
				...
				for(Entry<String, TestValues<V>> outerMap: loadcache.asMap().entrySet()){
					...
					for(Entry<String, TestValues<V>> innerMap: loadcache.asMap().entrySet()){
						...
						//System.out.println("outer visited nodes"+outerMap.getValue().getVisitedNodes());
						//System.out.println("inner visited nodes"+innerMap.getValue().getVisitedNodes());
						 
						 // compare two list and find disjunction element from oute to inner
						...
						if(outerMap.getKey().equals(innerMap.getKey()))
						 ...
						 Collection<?> diffElem = CollectionUtils.disjunction(outerMap.getValue().getVisitedNodes(), innerMap.getValue().getVisitedNodes());
							 if(diffElem.isEmpty())
								 continue;
							 
							 Set<Node> newSet= new HashSet <Node> ((Collection<? extends Node>) diffElem);
							 
							 System.out.println("dijunction elements: "+ newSet.size());
					...
					//mergeAllPaths.addAll(outerMap.getValue().getFullPaths());
						//System.err.println(outerMap.getValue().getFullPaths());
						//temp=outerMap.getValue().getFullPaths();
				...
				Collection<List<Edge<Node, Node>>> mergedPaths = new CunccurenListUpdate(this.startNode.toString(), this.targetNode.toString()).getCompletePath(mergeAllPaths, this.pathExp);
	...
	private void visit(TNode current) throws Exception {
		...
		if(LOCAL) {
			...
			for(StmtIterator stmtItr= gIdx.listStatements(gIdx.getResource(current.vertex.toString()), null, (RDFNode)null);stmtItr.hasNext(); ){
			...
			if(triple.getObject().equals(targetNode)) {
				...
				// if object of the current triple is not the actual target node call the remote requests
	   ...
	   private void getpathFromRemotesNewFunction(Set<Node> visitedNodes, List<String> beforeFilterEndp2) throws InterruptedException, ExecutionException {
				   ...
				   for(Node node: visitedNodes){
					   if(loadcache.asMap().get(endp).getVisitedNodes().contains(node)){
						   ...
						   // send remote request and update the cache
							/*get list of paths*/
							 Map<String,Set<Node>> pathsWithNodes = remoteRequest(node, targetNode, endp, 20);
							 ...
							 if(pathsWithNodes.isEmpty()){
								 ...
								 // filter out full and partial paths from this results and update cache accordingly
								 
								 for(Map.Entry<String, Set<Node>> pathsContnt: pathsWithNodes.entrySet()) {
									 
									 List<Node> firstLast= new ArrayList<>(pathsContnt.getValue());
									 // full paths
									 if(firstLast.get(0).equals(startNode.toString()) && firstLast.get(firstLast.size()-1).equals(targetNode.toString())) {
										...
										// System.err.println("partial paths"+loadcache.get(endp).getpPaths());
										 loadcache.get(endp).getVisitedNodes().addAll(pathsContnt.getValue());
										 }
									}
								 }
								
								 //System.out.println(loadcache.get(endp).getFullPaths());
							 }
							 ...
							 for(Map.Entry<String, Set<Node>> pathsContnt: pathsWithNodes.entrySet()) {
								 ...
								 if(firstLast.get(0).equals(startNode.toString()) && firstLast.get(firstLast.size()-1).equals(targetNode.toString())) {
									 	...
									 	if(!loadcache.asMap().containsKey(endp) ){
											 ...
											 TestValues<V> t1Values= new TestValues<V>(thisPathVisitedNodes, fullPathSingleLst, null);
									 ...
									 // partial paths
										 	...
										 	if(!loadcache.asMap().containsKey(endp) ){
												...
												TestValues<V> t1Values= new TestValues<V>(thisPathVisitedNodes, null, pPathSingleLst);
										 		loadcache.put(endp, t1Values);
										 		}else{
										 		if(loadcache.get(endp).getpPaths()==null){
	   ...
	   private  Map<String,Set<Node>> remoteRequest(Node node, Node targetNode2, String endp, int i) throws InterruptedException, ExecutionException{
